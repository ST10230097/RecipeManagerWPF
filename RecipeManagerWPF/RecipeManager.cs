...
namespace RecipeManagerWPF
{
    class RecipeManager
    {
        ...
        public event EventHandler<List<Recipe>> RecipeListUpdated;

        private void OnRecipeListUpdated(List<Recipe> recipes)
        {
            RecipeListUpdated?.Invoke(this, recipes);
        }

        public void FilterByIngredient(string ingredientName)
        {
            List<Recipe> filteredRecipes = recipes.FindAll(r => r.Ingredients.Any(i => i.Name.ToLower().Contains(ingredientName.ToLower())));
            OnRecipeListUpdated(filteredRecipes);
        }

        public void FilterByFoodGroup(string foodGroup)
        {
            List<Recipe> filteredRecipes = recipes.FindAll(r => r.Ingredients.Any(i => i.FoodGroup.ToLower() == foodGroup.ToLower()));
            OnRecipeListUpdated(filteredRecipes);
        }

        public void FilterByMaxCalories(int maxCalories)
        {
            List<Recipe> filteredRecipes = recipes.FindAll(r => r.Ingredients.Sum(i => i.Calories * i.Quantity) <= maxCalories);
            OnRecipeListUpdated(filteredRecipes);
        }
    }
